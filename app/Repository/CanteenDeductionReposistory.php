...
class CanteenDeductionReposistory
{
    ...
    {
        if($id > 0){
            $query = "SELECT leftside.*,IFNULL(canteen_bpn,0.00) canteen_bpn,IFNULL(canteen_bps,0.00) canteen_bps,IFNULL(canteen_agg,0.00) canteen_agg,remarks2 FROM (SELECT id,biometric_id,lastname,firstname,$id AS period_id FROM employees 
            WHERE emp_level = 6 AND exit_status = 1) AS leftside LEFT JOIN (
            SELECT canteen_bpn,canteen_bps,canteen_agg,biometric_id,period_id,remarks2 FROM unposted_weekly_compensation WHERE period_id = $id
